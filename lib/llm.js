...
export async function generateScenario({ topic, difficulty, steps }) {
  ...
  const system = [
    ...
    'PENTING: jawaban harus RINGKAS — objective & expected maksimal 1-2 kalimat, tiap clue maksimal 1 kalimat. Pastikan JSON ditutup lengkap.',
  ].join('\n');

  const user = [topicLine, `TINGKAT KESULITAN: ${diff}`, `JUMLAH LANGKAH: ${n}`].join('\n');

  // Retry transient upstream errors (e.g. 9Router 503/timeouts) before giving up.
  let parsed;
  let lastErr;
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
      ...
      lastErr = err;
      console.error(`generateScenario attempt ${attempt}/3 failed:`, err?.message || err);
      if (attempt < 3) await new Promise((r) => setTimeout(r, attempt * 1500));
    }
  }
  if (!parsed) throw lastErr || new Error('generateScenario gagal');
