...
// Active scenario for a room: AI-generated/saved JSONB takes precedence,
// otherwise fall back to a static scenario by id. (pg parses JSONB → object.)
export function getActiveScenario(room) {
  if (room?.scenario_data) return room.scenario_data;
  if (room?.scenario_id) return getScenario(room.scenario_id);
  return null;
}
